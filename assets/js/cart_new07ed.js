var spinner =
  '<td class="spinner"><div class="rect1"></div><div class="rect2"></div><div class="rect3"></div><div class="rect4"></div><div class="rect5"></div></td>';
...
function updateCart(event) {
  ...
  var queryString =
    "[data-productkey='" +
    currentItem.dataset.productkey +
    "'][data-color='" +
    currentItem.dataset.color +
    "'][data-prevsize='" +
    sizeEl.value +
    "']";
  if (
    event.target.className.includes("cartinput-size-select") &&
    ...
    $.ajax(settings).done(function (response) {
      ...
      if (json.hasOwnProperty("error")) {
        ...
        currentItem.querySelector(".cartinput-quantity").value =
          currentItem.dataset.prevqty;
        currentItem.querySelector(".cartinput-size-select").value =
          currentItem.dataset.prevsize;
      } else {
        ...
        if (
          ...
          currentItem.querySelector(".Price").innerHTML =
            "<span class='pull-right'>৳ " +
            (json.price - Math.floor((json.price * discount) / 100)) +
            "</span>" +
            '<div class="pull-right"><span style="color:#000">৳</span><span style="text-decoration: line-through;" class="cart_item_regular_price">' +
            json.regular_price +
            " </span>&nbsp;</div>";
          currentItem.querySelector(".cart_regular_price_field").textContent =
            json.regular_price * json.quantity + " ";
        } else {
          currentItem.querySelector(".Price").innerHTML =
            "<span class='pull-right'>৳ " +
            (json.price - Math.floor((json.price * discount) / 100)) +
        ...
        currentItem.querySelector(".cartinput-price").textContent =
          "৳ " +
          (json.price - Math.floor((json.price * discount) / 100)) *
            json.quantity;
        calcSum();
...
function updateCartBadge() {
  ...
  if (document.querySelectorAll(".new-designed-element").length == 0) {
    if (window.location.href.includes("cart")) window.location.href = "/shop";
  }
}
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        if (
          ...
          localStorage.getItem("currentcartsize") != null
        ) {
          ...
          if (discount == 0) {
            ...
            document.querySelector(".modal-price-text").innerText =
              ...
              calculateDynamicPrice(
                localStorage.getItem("currentcartcolor"),
                localStorage.getItem("currentcartsize")
              );
          } else {
            document.querySelector(".modal-discount-text").style.display =
              "block";
        ...
        document.querySelector(".pricemodal-header").innerHTML =
          '<p style= "text-align:center">Price Details</p>';
        document.querySelector(".pricemodal-footer").innerHTML =
          '<button class="modal-btn btn btn-outline-danger pull-left" data-dismiss="modal">Cancel</button></a>' +
          '<button class="discard-button modal-btn btn btn-outline-success pull-right" onclick="moreProductAddToCart(\'' +
          data[3]["id"] +
  ...
  variantInfo.ProductVariant.forEach(function (size) {
    ...
    if (!inventory.includes('"' + size.size + '"')) {
      ...
      if (product_inventory) {
        if (
          product_inventory.hasOwnProperty(size.size) &&
          product_inventory[size.size] > 0
        ) {
...
function colorHandler() {
  if (document.querySelector(".color-selector-selected")) {
    ...
    document.querySelector(".color-selector-selected").className =
...
function calculateDynamicPrice(color, size) {
      ...
      type.ProductVariant.forEach(function (e) {
        if (e.size == size) {
          calculatedPrice = parseInt(
            parseInt(localStorage.getItem("currentcartinkCost")) *
  ...
  return (
    Math.floor(calculatedPrice / 1) * 1 +
    parseInt(localStorage.getItem("cartaddedPrice"))
  );
}
...
function rePopulateSizeBoxes(color) {
  ...
  if (variantInfo.ProductVariant.length == 0)
    sizeSelectorDOM.innerHTML =
      "Sorry ! No sizes available at the moment.<br />";
  else {
    variantInfo.ProductVariant.forEach(function (size) {
      ...
      success: function (response) {
        if (response.result == "success") {
          ...
          if (response.hasOwnProperty("mark_product")) {
            product_value =
              response.mark_product.price * response.mark_product.quantity;
            response.mark_product.price = Number(
              response.mark_product.price
            ).toFixed(2);
            mtag("event", {
              type: "AddToCart",
              value: Number(product_value).toFixed(2),
        ...
        if ($(".shopping-cart").length > 0) {
          ...
          if (window.location.href.includes("/cart")) {
            setTimeout(function () {
              ...
              document.querySelector(".addToCartBtn > i").style.display =
                "none";
            }, 1000);
          } else {
            window.location.reload(false);
...
function showUnselectError() {
  ...
  if (document.querySelector(".color-selector-selected")) {
    document
      .querySelector("ul.list-inline.color-seletcor-container")
  ...
  if (document.querySelector(".size-selector-selected")) {
    document
      .querySelector("ul.size-selectors-container.list-inline")
      .classList.remove("error");
  } else {
    document
      .querySelector("ul.size-selectors-container.list-inline")
  ...
  if (showAddToCartBtn == false) {
    document.querySelector(
      "button.discard-button.modal-btn.btn.btn-outline-success.pull-right"
    ).disabled = true;
  } else {
    document.querySelector(
      "button.discard-button.modal-btn.btn.btn-outline-success.pull-right"
