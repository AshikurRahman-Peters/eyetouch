    ...
    <div class="container-fluid">
        ...
        <div class="teescontainer col-sm-12 col-md-11 col-lg-11" style="min-height: 90vh; margin-top: 40px !important;">
                            ...
                            <div class="ais-Panel-body">
                                <div>
                                    <div class="ais-RefinementList">
                                        <ul class="ais-RefinementList-list" id="categoryFilter">
                                            <?php
                                            ...
                                            <li class="ais-RefinementList-item">
                                                <div>
                                                    <div class="facet-item checkitem newarrival level1">
                                                        <input type="checkbox"
                                                            class="ais-RefinementList-checkbox usual-checkbox"
                                                            value="<?php echo url_title($categoryValue->cat_name, '-', true); ?>"
                                                            onchange="filtercatProducts()">
                                                        <?php echo $categoryValue->cat_name; ?>
                                                    </div>
                                                </div>
                                            </li>
                                            <?php
                                            $sub_category = $this->db->query("select * from tbl_sub_category where status='a' and category_id ='$categoryValue->cat_id'")->result();
                                            foreach ($sub_category as $subCategoryValue) {
                                            ?>
                                            <li class="ais-RefinementList-item"
                                                data-category="<?php echo url_title($categoryValue->cat_name, '-', true).'-'.url_title($subCategoryValue->sub_category_name, '-', true);?>">
                                                <div>
                                                    <div class="facet-item checkitem level2">
                                                        <input type="checkbox"
                                                            class="ais-RefinementList-checkbox usual-checkbox"
                                                            value="<?php echo url_title($categoryValue->cat_name, '-', true).'-'.url_title($subCategoryValue->sub_category_name, '-', true);?>"
            ...
            <div class="resultset col-sm-12 col-md-12 col-lg-9">
                ...
                <!-- <span id="exampleText" style="text-transform:uppercase;font-size: 14px" class="btn btn-primery">
                </span> -->
                <li class="ais-CurrentRefinements-item" id="exampleText"
                    ...
                    <div class="results-wrapper">
                        ...
                        <main id="hits">
                            <div class="ais-InfiniteHits">
                                <ol class="ais-InfiniteHits-list" id="productList">
                                    <?php
                                if (isset($AllProductData)) {
                                    foreach ($AllProductData as $product) {
                                    ?>
                                    <li class="ais-InfiniteHits-item"
                                        data-category="<?php echo url_title($product->cat_name, '-', true);?>"
                                        data-subcategory="<?php echo url_title($product->cat_name, '-', true).'-'.url_title($product->sub_category_name, '-', true);?>">
                                        <div style="position:relative"
                                            ...
                                            <div style="display:none" class="sale"><span>Sale</span></div>
            ...
            function filterProducts() {
                // Get all checkboxes
                var checkboxes = document.querySelectorAll('#categoryFilter input[type="checkbox"]');
                ...
                var productItems = document.querySelectorAll('#productList li');
                var categoryDisplay = document.getElementById('exampleText');

                // Initialize an empty string to store the selected categories
                var selectedCategoriesText = '';
                // Loop through product items
                productItems.forEach(function(productItem) {
                    // Get the category of each product item

                    var productCategory = productItem.dataset.subcategory;

                    // Check if the product category is in the selected categories
                    if (selectedCategories.length === 0 || selectedCategories.includes(productCategory)) {
                        selectedCategoriesText += productCategory +
                            ' > ';
                        productItem.style.display = 'contents'; // Show the item
                    } else {
                        productItem.style.display = 'none'; // Hide the item
                    }
                });
                const maxLength = 142; // Set your desired maximum length

                let truncatedText = selectedCategoriesText.slice(0, maxLength);

                if (selectedCategoriesText.length > maxLength) {
                    truncatedText += '...'; // Add ellipsis or any indicator for truncated text
                }

                categoryDisplay.innerHTML =
                    `<span class="ais-CurrentRefinements-categoryLabel">${truncatedText}</span>`;
            }
            ...
            function filtercatProducts() {
                ...
                // Get all product items
                var productItems = document.querySelectorAll('#productList li');
                // Assuming you have an element to display the selected categories
                var categoryDisplay = document.getElementById('exampleText');
                ...
                productItems.forEach(function(productItem) {
                    ...
                    var productCategory = productItem.dataset.category;

                    // Check if the product category is in the selected categories
                    if (selectedCategories.length === 0 || selectedCategories.includes(productCategory)) {
                        // Concatenate the category text
                        selectedCategoriesText += productCategory +
                            ' > ';
